...
export class TurnService {
    ...
    private baseUrl: string = environment.apiUrl + '/turn';

    httpOptions = {
        headers: new HttpHeaders({ 'Content-Type': 'application/json' }),
    };

    getAll(): Observable<{ status: boolean; data: Turn[]; }> {
        return this.http
        .get<any>(this.baseUrl, this.httpOptions)
        .pipe(retry(1), catchError(this.errorService.errorHandler));
    }

    getById(id: number): Observable<{ status: boolean; data: Turn; }> {
        return this.http
